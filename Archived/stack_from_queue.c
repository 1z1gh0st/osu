/***********************************************************
* Author: Chenyu Song and Philip Warton
...
void listQueueInit(struct Queue* queue) 
{
	...
	assert(queue != NULL);
	struct Link *qu_link = (struct Link*) malloc(sizeof(struct Link));
	assert(qu_link != NULL);
	qu_link -> next =NULL;
	queue->head = qu_link;
	queue->tail = qu_link; 
...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
	...
	assert(queue != NULL);
	assert(!listQueueIsEmpty(queue));
	struct Link* node = queue->head->next;
	TYPE value = node->value;
	if(queue->head->next == queue->tail){
		queue->tail = queue->head;
		queue->head->next = NULL;
	}
	else queue->head->next = queue->head->next->next;
...
int listStackIsEmpty(struct Stack* stack)
{
	...
	/* FIXME: You will write this function */
	assert(stack != NULL);
	if(stack->q1->head->next == NULL) return 1;
	else return 0;
}
...
void listStackPush(struct Stack* stack, TYPE value) 
{
	...
	while(!listQueueIsEmpty(stack->q1)){
		tmp = listQueueRemoveFront(stack->q1);
