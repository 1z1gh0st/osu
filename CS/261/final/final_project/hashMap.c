...
void hashMapWrite(HashMap* map, FILE* file) {
	...
	for (i = 0; i < map->capacity; i++) {
		if (map->table[i] != NULL) {
			...
			while (curr != NULL) {
				printf(
						"%s %s %s %02d/%02d/%04d %1d\n",
						curr->value->first_name,
						curr->value->last_name,
						curr->value->id,
						curr->value->date_started.month,
						curr->value->date_started.day,
						curr->value->date_started.year,
						curr->value->currently_employed);
				curr = curr->next;
			}
		}
	}
