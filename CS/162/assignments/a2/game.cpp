...
void Game::computer_turn(Hand &_table_pile_) {
    ...
    while (!check_win_conditions()) {
        ...
        if (p2_index < 0) {
            std::cout << "Computer has no valid cards, will draw... :(\n";
            p2_index = 0;
        }
...
bool Game::check_win_conditions() {
    ...
    if (cards.get_n_cards() == 0) {
        if (players[0].get_n_cards() > players[1].get_n_cards()) {
            std::cout << "Computer Wins --- Deck ran out and you had more cards than computer" << std::endl;
            return true;
        } else if (players[0].get_n_cards() < players[1].get_n_cards()) {
            std::cout << "Player Wins --- Deck ran out and you had fewer cards" << std::endl; 
            return true;
        } else {
            std::cout << "Game Drawn --- Deck ran out and each player had equal number of cards\n";
            return true;
        }
    } else if (players[0].get_n_cards() == 0) {
        std::cout << "Player Wins --- Player has 0 cards in hand" << std::endl;
        return true;
    } else if (players[1].get_n_cards() == 0) {
        std::cout << "Computer Wins --- Computer has 0 cards in hand" << std::endl;
        return true;
    } else {
        return false;
    }
