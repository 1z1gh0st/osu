...
def main():
    ...
    # Take previous from before and apply it on test data
    X_test_bias = dummyAugment(X_test)
    y_pred_test = X_test_bias @ w >= 0

    # Make id number vector
    n = len(y_pred_test)
    idx = np.array(np.zeros((n, 1)))
    for i in range(len(idx)):
        idx[i] = [i]

    # Stick id's to front of array
    result = np.hstack((idx, y_pred_test))

    # Export csv file
    np.savetxt("test_predictions.csv", result, delimiter=",", header="id,type", fmt='%i', comments='')
...
def logistic(z):
    ...
    # By the definition of the logistic function we write
...
def calculateNegativeLogLikelihood(X, y, w):
    ...
    # We define the negative log likelihood by the following using matrices
...
def trainLogistic(X, y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        # We compute the gradient over the dataset and store in w_grad
...
def kFoldCrossVal(X, y, k):
    ...
    for j in range(k):
        ...
        train_idx = np.concatenate(
            [np.arange(0, start), np.arange(end, len(X))]
        )
        if len(test_idx) < 2:
            break
