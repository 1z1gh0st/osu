...
def main():
  ...
  # Get the length of our test set
  n = len(X_test)

  # Make a column vector of the row number for the 'id' column
  rowNums = np.arange(0,n)
  rowNums = rowNums[:,None]

  # Run the neural net on the test set
  results = np.argmax(net.forward(X_test), axis=1)[:,np.newaxis]

  # Put together our column vector of row numbers and predicted classes together
  results = np.hstack(
    (rowNums, results)
    )

  # Output to csv file with desired formatting
  np.savetxt("test_predictions.csv", results, delimiter=",", header="id,digit", fmt='%i', comments='')
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    # A datapoint X is given by self.input
    self.grad_weights = self.input.T @ grad

    # We take the sum along each row, thus axis=0
    self.grad_bias = np.sum(grad, axis=0)

    # We finally return the desired result as follows
